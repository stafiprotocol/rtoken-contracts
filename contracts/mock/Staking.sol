...
contract Staking is IStaking, System, IParamSubscriber {
  ...
  function handleDelegateAckPackage(address delegator,address validator, uint256 amount, uint8 status, uint8 errCode) public {
    delegateInFly[delegator] -= 1;
    if (status == CODE_SUCCESS) {
      require(errCode == 0, "wrong status");
      delegated[delegator] = delegated[delegator].add(amount);
  ...
  function handleDistributeRewardSynPackage(address recipient, uint256 amount) public payable returns(uint32, bytes memory) {
    distributedReward[recipient] = distributedReward[recipient].add(amount);
